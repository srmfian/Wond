...
def audio_quality(settings: Settings, path: Path, *, min_seconds: float | None = None) -> dict[str, Any]:
    sample_rate = int(audio_preprocessing_config(settings).get("quality_sample_rate") or 16000)
    ...
    if min_duration is None:
        min_duration = float(audio_preprocessing_config(settings).get("quality_min_seconds") or 0.8)
    min_rms = float(audio_preprocessing_config(settings).get("quality_min_rms_dbfs") or -45.0)
    max_clip = float(audio_preprocessing_config(settings).get("quality_max_clipped_ratio") or 0.02)
    ok = duration >= min_duration and rms_dbfs >= min_rms and clipped_ratio <= max_clip
    reason = None
